  ...
  return (
    <div className="p-6">
      <div className="flex items-center justify-between mb-6">
        <div>
          <h1 className="text-2xl font-bold tracking-tight text-gray-900 dark:text-gray-100">Maintenance Logging</h1>
          <p className="mt-1 text-sm text-gray-600 dark:text-gray-400">Track shop repairs and check-ups for fleet assets.</p>
        </div>
      ...
      {isLogsLoading ? (
        <div className="py-12 text-center text-gray-500 dark:text-gray-400 font-medium">Loading shop records...</div>
      ) : (
            ...
            <table className="w-full text-left border-collapse text-sm">
              <thead>
                <tr className="border-b border-gray-300 dark:border-gray-700 bg-gray-50 dark:bg-gray-900 font-semibold text-gray-700 dark:text-gray-300 uppercase tracking-wider text-[11px]">
                  ...
                  <th className="px-6 py-3 border-r border-gray-200 dark:border-gray-700">Issue Description</th>
                  <th className="px-6 py-3 border-r border-gray-200 dark:border-gray-700">Opened At</th>
                  <th className="px-6 py-3 border-r border-gray-200 dark:border-gray-700">Closed At</th>
              ...
              <tbody className="divide-y divide-gray-200 dark:divide-gray-700">
                {logs.length === 0 ? (
                  <tr>
                    <td colSpan={6} className="px-6 py-10 text-center text-gray-500 dark:text-gray-400 font-medium">
                  ...
                  filteredLogs.map((log) => (
                    <tr key={log.id} className="hover:bg-gray-50 dark:hover:bg-gray-700/50">
                      <td className="px-6 py-4 border-r border-gray-200 dark:border-gray-700">
                        <div className="font-semibold text-gray-900 dark:text-gray-100">{log.vehicle.name}</div>
                        <div className="text-xs font-mono text-gray-500 dark:text-gray-400 mt-0.5">{log.vehicle.regNumber}</div>
                      </td>
                      <td className="px-6 py-4 text-gray-700 dark:text-gray-300 border-r border-gray-200 dark:border-gray-700 font-medium whitespace-pre-wrap max-w-xs">{log.description}</td>
                      <td className="px-6 py-4 text-gray-600 dark:text-gray-400 border-r border-gray-200 dark:border-gray-700 text-xs">{formatDateDisplay(log.openedAt)}</td>
                      <td className="px-6 py-4 text-gray-600 dark:text-gray-400 border-r border-gray-200 dark:border-gray-700 text-xs">{formatDateDisplay(log.closedAt)}</td>
                      <td className="px-6 py-4 text-center border-r border-gray-200 dark:border-gray-700">
                        {log.active ? (
                          <span className="px-2.5 py-1 text-xs font-semibold uppercase tracking-wider border border-amber-200 dark:border-amber-900/50 bg-amber-50 dark:bg-amber-950/30 text-amber-800 dark:text-amber-400">
                            Active
                          </span>
                        ) : (
                          <span className="px-2.5 py-1 text-xs font-semibold uppercase tracking-wider border border-green-200 dark:border-green-900/50 bg-green-50 dark:bg-green-950/30 text-green-800 dark:text-green-400">
                            Closed
                          </span>
                        )}
                      </td>
                      <td className="px-6 py-4 text-center">
                        {log.active ? (
                          <button
                            ...
                            className="inline-flex items-center gap-1 border border-green-300 dark:border-green-900 bg-green-50 dark:bg-green-950/20 px-2.5 py-1 text-xs font-semibold text-green-700 dark:text-green-400 hover:bg-green-100 dark:hover:bg-green-900/40"
                          >
                            <CheckSquare className="h-3 w-3" />
                            CLOSE LOG
                          </button>
                        ) : (
                          <span className="text-xs text-gray-500 dark:text-gray-400 font-medium italic">Completed</span>
                        )}
                      </td>
                    </tr>
                  ))
      ...
      {isModalOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-40 dark:bg-opacity-60 p-4">
          <div className="w-full max-w-md border border-gray-300 dark:border-gray-700 bg-white dark:bg-gray-800 p-6 shadow-md relative">
            ...
            <div className="mb-6">
              <h2 className="text-lg font-bold text-gray-900 dark:text-gray-100">Log Vehicle Maintenance</h2>
              <p className="text-xs text-gray-500 dark:text-gray-400 mt-1">Place an asset in shop repairs. This sets vehicle status to IN_SHOP.</p>
            </div>
            ...
            <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
              ...
              <div>
                ...
                <textarea
                  ...
                  className="mt-1 block w-full border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-900 px-3 py-2 text-sm text-gray-900 dark:text-gray-100 placeholder-gray-400 dark:placeholder-gray-500 focus:border-black dark:focus:border-white focus:outline-none resize-none"
                />
                {errors.description && (
                  <p className="mt-1 text-xs text-red-600 font-medium">
