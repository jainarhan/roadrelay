...
export const MyTrips: React.FC = () => {
  ...
  if (!user?.driverId) {
    return (
      <div className="p-6">
        <div className="mb-6">
          <h1 className="text-2xl font-bold tracking-tight text-gray-900">My Trips</h1>
          <p className="mt-1 text-sm text-gray-600">View and track your assigned delivery routes.</p>
        </div>
        <div className="border border-amber-300 bg-amber-50 p-6 text-sm text-amber-800 shadow-sm font-semibold">
  ...
  return (
    <div className="p-6">
      ...
      {isLoading ? (
        <div className="py-12 text-center text-gray-500 font-medium">Loading your trip assignments...</div>
      ) : (
          ...
          <table className="w-full text-left border-collapse text-sm">
            <thead>
              <tr className="border-b border-gray-300 bg-gray-50 font-semibold text-gray-700 uppercase tracking-wider text-[11px]">
                <th className="px-6 py-3 border-r border-gray-200">Source</th>
                <th className="px-6 py-3 border-r border-gray-200">Destination</th>
                <th className="px-6 py-3 border-r border-gray-200 text-right">Cargo Weight</th>
                <th className="px-6 py-3 border-r border-gray-200">Status</th>
              ...
              {myTrips.length === 0 ? (
                ...
                myTrips.map((t) => (
                  <tr key={t.id} className="hover:bg-gray-50">
                    <td className="px-6 py-4 border-r border-gray-200 font-medium text-gray-900">{t.source}</td>
                    <td className="px-6 py-4 border-r border-gray-200 font-medium text-gray-900">{t.destination}</td>
                    <td className="px-6 py-4 border-r border-gray-200 text-right font-medium text-gray-900">
                      {t.cargoWeight.toLocaleString()} kg
                    </td>
                    <td className="px-6 py-4 border-r border-gray-200">{getStatusBadge(t.status)}</td>
                    <td className="px-6 py-4 font-medium text-gray-900">{formatDateDisplay(t.dispatchedAt)}</td>
                  </tr>
                ))
              )}
            </tbody>
